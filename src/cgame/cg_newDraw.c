...
static void CG_DrawPlayerArmorValue( rectDef_t *rect, int font, float scale, vec4_t color, qhandle_t shader, int textStyle ) {
	...
	if ( shader ) {
		...
		CG_DrawPic( rect->x, rect->y, rect->w, rect->h, shader, UI_CENTER );
		trap_R_SetColor( NULL );
	} else {
		Com_sprintf( num, sizeof( num ), "%i", value );
		value = CG_Text_Width( num, font, scale, 0 );
		CG_Text_Paint( rect->x + ( rect->w - value ) / 2, rect->y + rect->h, font, scale, color, num, 0, 0, textStyle, UI_BOTTOM );
	}
}
...
static void CG_DrawPlayerWeaponIcon( rectDef_t *rect, qboolean drawHighlighted, int align ) {
	...
	if ( icon ) {
		...
		CG_DrawPic( x, y, w, h, icon, UI_BOTTOM );
	}
}
...
static void CG_DrawPlayerAmmoIcon( rectDef_t *rect, qboolean draw2D ) {
	...
	if ( draw2D || ( !cg_draw3dIcons.integer && cg_drawIcons.integer ) ) {
		...
		if ( icon ) {
			CG_DrawPic( rect->x, rect->y, rect->w, rect->h, icon, UI_CENTER );
		}
	} else if ( cg_draw3dIcons.integer ) {
...
static void CG_DrawCursorhint( rectDef_t *rect ) {
	...
	trap_R_SetColor( color );
	CG_DrawPic( rect->x - halfscale, rect->y - halfscale, rect->w + scale, rect->h + scale, icon, UI_CENTER );

	if ( icon2 ) {
		CG_DrawPic( rect->x - halfscale, rect->y - halfscale, rect->w + scale, rect->h + scale, icon2, UI_CENTER );
	}
...
static void CG_DrawMessageIcon( rectDef_t *rect ) {
	...
	CG_DrawPic( rect->x, rect->y, rect->w, rect->h, icon, UI_BOTTOM );
}
...
//				value2 = ps->ammoclip[weapAlts[weap]];
	...
	if ( value > -1 ) {
		if ( shader ) {
			trap_R_SetColor( color );
			CG_DrawPic( rect->x, rect->y, rect->w, rect->h, shader, UI_BOTTOM );
			trap_R_SetColor( NULL );
		} else {
			Com_sprintf( num, sizeof( num ), "%i", value );
			value = CG_Text_Width( num, font, scale, 0 );
...
//			if(special) {	// draw '0' for akimbo guns
			if ( value2 || ( special && type == 1 ) ) {
				...
				value = CG_Text_Width( num, font, scale, 0 );
				CG_Text_Paint( -30 + rect->x + ( rect->w - value ) / 2, rect->y + rect->h, font, scale, color, num, 0, 0, textStyle, UI_BOTTOM );
			}
		}
	}
}
...
static void CG_DrawSelectedPlayerHealth( rectDef_t *rect, int font, float scale, vec4_t color, qhandle_t shader, int textStyle ) {
	...
	if ( ci ) {
		if ( shader ) {
			...
			value = CG_Text_Width( num, font, scale, 0 );
			CG_Text_Paint( rect->x + ( rect->w - value ) / 2, rect->y + rect->h, font, scale, color, num, 0, 0, textStyle, UI_CENTER );
		}
	}
}
...
//	ci = cgs.clientinfo + ((voice) ? cgs.currentVoiceClient : sortedTeamPlayers[CG_GetSelectedPlayer()]);
	ci = cgs.clientinfo + sortedTeamPlayers[CG_GetSelectedPlayer()];
	if ( ci ) {
		CG_Text_Paint( rect->x, rect->y + rect->h, font, scale, color, ci->name, 0, 0, textStyle, UI_CENTER );
	}
}
...
static void CG_DrawSelectedPlayerLocation( rectDef_t *rect, int font, float scale, vec4_t color, int textStyle ) {
	...
	if ( ci ) {
		...
		CG_Text_Paint( rect->x, rect->y + rect->h, font, scale, color, p, 0, 0, textStyle, UI_CENTER );
	}
}
...
static void CG_DrawSelectedPlayerWeapon( rectDef_t *rect ) {
	...
	if ( ci ) {
		if ( cg_weapons[ci->curWeapon].weaponIcon[1] ) {
			CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cg_weapons[ci->curWeapon].weaponIcon[1], UI_CENTER );  // (SA) using the 'selected' version of the icon
		} else {
			CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cgs.media.deferShader, UI_CENTER );
		}
	}
}
...
//			CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cg_items[ value ].icons[2-(value-1)] );
			CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cg_items[item - bg_itemlist].icons[2 - ( value - 1 )], UI_CENTER );
		} else {
//			CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cg_items[ value ].icons[0] );
			CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cg_items[item - bg_itemlist].icons[0], UI_CENTER );
		}
	}
}
...
static void CG_DrawPlayerItem( rectDef_t *rect, int font, float scale, qboolean draw2D ) {
	...
	if ( value ) {
		...
		if ( qtrue ) {
			CG_RegisterItemVisuals( value );
			CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cg_items[ value ].icons[0], UI_CENTER );
		} else {
			VectorClear( angles );
			origin[0] = 90;
			origin[1] = 0;
	...
	if ( ci ) {
		...
		for ( j = 0; j < PW_NUM_POWERUPS; j++ ) {
			if ( ci->powerups & ( 1 << j ) ) {
				...
				if ( item ) {
					CG_DrawPic( x, y, rect->w, rect->h, trap_R_RegisterShader( item->icon ), UI_CENTER );
					x += 3;
					y += 3;
					return;
				}
...
//	ci = cgs.clientinfo + ((voice) ? cgs.currentVoiceClient : sortedTeamPlayers[CG_GetSelectedPlayer()]);
	...
	if ( ci ) {
		if ( cg_draw3dIcons.integer ) {
			...
			CG_DrawPic( rect->x, rect->y, rect->w, rect->h, ci->modelIcon, UI_CENTER );
		}
...
static void CG_DrawRedScore( rectDef_t *rect, int font, float scale, vec4_t color, qhandle_t shader, int textStyle ) {
	...
	value = CG_Text_Width( num, font, scale, 0 );
	CG_Text_Paint( rect->x + rect->w - value, rect->y + rect->h, font, scale, color, num, 0, 0, textStyle, UI_CENTER );
}
...
static void CG_DrawAreaPowerUp( rectDef_t *rect, int align, float spacing, int font, float scale, vec4_t color ) {
	...
	for ( i = 0 ; i < active ; i++ ) {
		...
		if ( item ) {
			...
			CG_DrawPic( r2.x, r2.y, r2.w * .75, r2.h, trap_R_RegisterShader( item->icon ), UI_CENTER );

			Com_sprintf( num, sizeof( num ), "%i", sortedTime[i] / 1000 );
			CG_Text_Paint( r2.x + ( r2.w * .75 ) + 3, r2.y + r2.h, font, scale, color, num, 0, 0, 0, UI_CENTER );
			*inc += r2.w + spacing;
		}
...
static void CG_DrawAreaSystemChat( rectDef_t *rect, int font, float scale, vec4_t color, qhandle_t shader ) {
	CG_Text_Paint( rect->x, rect->y + rect->h, font, scale, color, systemChat, 0, 0, 0, UI_CENTER );
}

static void CG_DrawAreaTeamChat( rectDef_t *rect, int font, float scale, vec4_t color, qhandle_t shader ) {
	CG_Text_Paint( rect->x, rect->y + rect->h, font, scale, color,teamChat1, 0, 0, 0, UI_CENTER );
}

static void CG_DrawAreaChat( rectDef_t *rect, int font, float scale, vec4_t color, qhandle_t shader ) {
	CG_Text_Paint( rect->x, rect->y + rect->h, font, scale, color, teamChat2, 0, 0, 0, UI_CENTER );
}
...
static void CG_DrawKiller( rectDef_t *rect, int font, float scale, vec4_t color, qhandle_t shader, int textStyle ) {
	...
	if ( cg.killerName[0] ) {
		...
		CG_Text_Paint( x - CG_Text_Width( CG_GetKillerText(),font, scale, 0 ) / 2, rect->y + rect->h, font, scale, color, CG_GetKillerText(), 0, 0, textStyle, UI_CENTER );
	}
...
static void CG_DrawCapFragLimit( rectDef_t *rect, int font, float scale, vec4_t color, qhandle_t shader, int textStyle ) {
	...
	CG_Text_Paint( rect->x, rect->y, font, scale, color, va( "%2i", limit ),0, 0, textStyle, UI_CENTER );
}

static void CG_Draw1stPlace( rectDef_t *rect, int font, float scale, vec4_t color, qhandle_t shader, int textStyle ) {
	if ( cgs.scores1 != SCORE_NOT_PRESENT ) {
		CG_Text_Paint( rect->x, rect->y, font, scale, color, va( "%2i", cgs.scores1 ),0, 0, textStyle, UI_CENTER );
	}
}

static void CG_Draw2ndPlace( rectDef_t *rect, int font, float scale, vec4_t color, qhandle_t shader, int textStyle ) {
	if ( cgs.scores2 != SCORE_NOT_PRESENT ) {
		CG_Text_Paint( rect->x, rect->y, font, scale, color, va( "%2i", cgs.scores2 ),0, 0, textStyle, UI_CENTER );
	}
}
...
static void CG_DrawGameStatus( rectDef_t *rect, int font, float scale, vec4_t color, qhandle_t shader, int textStyle ) {
	CG_Text_Paint( rect->x, rect->y + rect->h, font, scale, color, CG_GetGameStatusText(), 0, 0, textStyle, UI_CENTER );
}
...
static void CG_DrawGameType( rectDef_t *rect, int font, float scale, vec4_t color, qhandle_t shader, int textStyle ) {
	CG_Text_Paint( rect->x, rect->y + rect->h, font, scale, color, CG_GameTypeString(), 0, 0, textStyle, UI_CENTER );
}
...
void CG_DrawMedal( int ownerDraw, rectDef_t *rect, int font, float scale, vec4_t color, qhandle_t shader ) {
	...
	if ( text ) {
		...
		CG_Text_Paint( rect->x + ( rect->w - value ) / 2, rect->y + rect->h + 10, font, scale, color, text, 0, 0, 0, UI_CENTER );
	}
	trap_R_SetColor( NULL );
