...
static void CG_DrawClientScore( int x, int y, score_t *score, float *color, float fade ) {
	...
	if ( ci->botSkill > 0 && ci->botSkill <= 5 ) {
		CG_DrawPic( 0, y - 8, 32, 32, cgs.media.botSkillShaders[ ci->botSkill - 1 ], UI_CENTER );
	...
	if ( score->client == cg.snap->ps.clientNum ) {
		...
		hcolor[3] = fade * 0.7;
		CG_FillRect( x - 2, y,  SCOREBOARD_WIDTH, BIGCHAR_HEIGHT + 1, hcolor, UI_CENTER );
	}
...
static void WM_DrawClientScore( int x, int y, score_t *score, float *color, float fade ) {
	...
	if ( score->client == cg.snap->ps.clientNum ) {
		...
		CG_FillRect( tempx, y + 1, INFO_PLAYER_WIDTH - INFO_BORDER, SMALLCHAR_HEIGHT - 1, hcolor, UI_CENTER );
		tempx += INFO_PLAYER_WIDTH;

		CG_FillRect( tempx, y + 1, INFO_SCORE_WIDTH - INFO_BORDER, SMALLCHAR_HEIGHT - 1, hcolor, UI_CENTER );
		tempx += INFO_SCORE_WIDTH;

		CG_FillRect( tempx, y + 1, INFO_LATENCY_WIDTH - INFO_BORDER, SMALLCHAR_HEIGHT - 1, hcolor, UI_CENTER );
...
static int WM_TeamScoreboard( int x, int y, team_t team, float fade ) {
	...
	CG_FillRect( tempx, y, INFO_PLAYER_WIDTH - INFO_BORDER, INFO_TEAM_HEIGHT, hcolor, UI_CENTER );
	if ( team == TEAM_RED ) {
	...
	CG_FillRect( tempx, y, INFO_SCORE_WIDTH - INFO_BORDER, INFO_TEAM_HEIGHT, hcolor, UI_CENTER );
	tempx += INFO_SCORE_WIDTH;

	CG_FillRect( tempx, y, INFO_LATENCY_WIDTH - INFO_BORDER, INFO_TEAM_HEIGHT, hcolor, UI_CENTER );
	tempx += INFO_LATENCY_WIDTH;
...
int WM_DrawObjectives( int x, int y, int width, float fade ) {
	...
	if ( buf && atoi( buf ) ) {
		...
		for ( i = 0; i < num; i++ ) {
			...
			if ( status == 0 ) {
				CG_DrawPic( x + width / 2 - strwidth / 2 - 16 - 24, y, 24, 16, trap_R_RegisterShaderNoMip( "ui/assets/ger_flag.tga" ), UI_CENTER );
				CG_DrawPic( x + width / 2 + strwidth / 2 - 12 + 4, y, 24, 16, trap_R_RegisterShaderNoMip( "ui/assets/ger_flag.tga" ), UI_CENTER );
			} else if ( status == 1 )   {
				CG_DrawPic( x + width / 2 - strwidth / 2 - 16 - 24, y, 24, 16, trap_R_RegisterShaderNoMip( "ui/assets/usa_flag.tga" ), UI_CENTER );
				CG_DrawPic( x + width / 2 + strwidth / 2 - 12 + 4, y, 24, 16, trap_R_RegisterShaderNoMip( "ui/assets/usa_flag.tga" ), UI_CENTER );
			}
...
int WM_ScoreboardOverlay( int x, int y, float fade ) {
	...
	// draw background
	CG_FillRect( x - 12, y, width, 400, hcolor, UI_CENTER );

	// draw title frame
	VectorSet( hcolor, 0.0039, 0.0039, 0.2461 );
	hcolor[3] = 1 * fade;
	CG_FillRect( x - 12, y, width, 30, hcolor, UI_CENTER );
	CG_DrawRect( x - 12, y, width, 400, 2, hcolor, UI_CENTER );
...
qboolean CG_DrawScoreboard( void ) {
	...
	#endif
		CG_DrawPic( x + 1 * 16, y, 64, 32, cgs.media.scoreboardScore, UI_CENTER );
		CG_DrawPic( x + 6 * 16 + 8, y, 64, 32, cgs.media.scoreboardPing, UI_CENTER );
		CG_DrawPic( x + 11 * 16 + 8, y, 64, 32, cgs.media.scoreboardTime, UI_CENTER );
		CG_DrawPic( x + 16 * 16, y, 64, 32, cgs.media.scoreboardName, UI_CENTER );
...
void CG_DrawTourneyScoreboard( void ) {
	...
	color[3] = 1;
	CG_FillRect( 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, color, UI_CENTER );
