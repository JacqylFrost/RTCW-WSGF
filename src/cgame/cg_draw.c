...
void CG_Text_PaintChar( float x, float y, float width, float height, float scale, float s, float t, float s2, float t2, qhandle_t hShader, int mode ) {
	float w, h;
	w = width * scale;
	h = height * scale;
	CG_ScaleFrom640( &x, &y, &w, &h, mode );
	trap_R_DrawStretchPic( x, y, w, h, s, t, s2, t2, hShader );
}

void CG_Text_Paint( float x, float y, int font, float scale, vec4_t color, const char *text, float adjust, int limit, int style, int mode ) {
	int len, count;
	vec4_t newColor;
	glyphInfo_t *glyph;
	float useScale;
...
//		CG_DrawPic( x, y, w, h, ci->modelIcon );
	...
	if ( ci->deferred ) {
		CG_DrawPic( x, y, w, h, cgs.media.deferShader, UI_CENTER );
	}
}
...
void CG_DrawTeamBackground( int x, int y, int w, int h, float alpha, int team ) {
	...
	CG_DrawPic( x, y, w, h, cgs.media.teamStatusBar, UI_CENTER );
	trap_R_SetColor( NULL );
}
	...
	for ( i = 0; i < numSortedTeamPlayers; i++ ) {
		...
		if ( ci->infoValid && ci->team == cg.snap->ps.persistant[PERS_TEAM] ) {
			...
			CG_DrawPic( xx, y, TINYCHAR_WIDTH, TINYCHAR_HEIGHT,
						cg_weapons[ci->curWeapon].weaponIcon[0], UI_CENTER );
			...
			for ( j = 0; j < PW_NUM_POWERUPS; j++ ) {
				if ( ci->powerups & ( 1 << j ) ) {
					...
					if ( item != NULL ) { // JPW NERVE added for invulnerability powerup at beginning of map
						CG_DrawPic( xx, y, TINYCHAR_WIDTH, TINYCHAR_HEIGHT,
									trap_R_RegisterShader( item->icon ), UI_CENTER );
						xx -= TINYCHAR_WIDTH;
					} // jpw
				}
			}
...
#define CHATLOC_X 0
	...
	if ( cgs.teamLastChatPos != cgs.teamChatPos ) {
		...
		trap_R_SetColor( hcolor );
		CG_DrawPic( CHATLOC_X, CHATLOC_Y - h, 640, h, cgs.media.teamStatusBar, UI_CENTER );
		trap_R_SetColor( NULL );
...
//			CG_DrawPic( 100, (SCREEN_HEIGHT-ICON_SIZE)-8, ICON_SIZE/2, ICON_SIZE, cg_items[item - bg_itemlist].icons[2-(value-1)] );
			CG_DrawPic( 606, 366, 24, 48, cg_items[item - bg_itemlist].icons[2 - ( value - 1 )], UI_CENTER );

		} else {
//			CG_DrawPic( 100, (SCREEN_HEIGHT-ICON_SIZE)-8, ICON_SIZE/2, ICON_SIZE, cg_items[item - bg_itemlist].icons[0] );
			CG_DrawPic( 606, 366, 24, 48, cg_items[item - bg_itemlist].icons[0], UI_CENTER );

		}

		// draw the selection box so it's not just floating in space
		CG_DrawPic( 606 - 4, 366 - 4, 32, 56, cgs.media.selectShader, UI_CENTER );
	}
}
/*
		if(cg.holdableSelect == HI_WINE) {
...
static void CG_DrawReward( void ) {
	...
	for ( i = 0 ; i < cg.rewardCount ; i++ ) {
		CG_DrawPic( x, y, ICON_SIZE - 4, ICON_SIZE - 4, cg.rewardShader, UI_CENTER );
		x += ICON_SIZE;
	}
	trap_R_SetColor( NULL );
}
...
static void CG_DrawDisconnect( void ) {
	...
	CG_DrawPic( x, y, 48, 48, trap_R_RegisterShader( "gfx/2d/net.tga" ), UI_CENTER );
}
...
//	if(0) {
	...
	trap_R_SetColor( NULL );
	CG_DrawPic( x, y, 48, 48, cgs.media.lagometerShader, UI_CENTER );
...
static void CG_DrawWeapReticle( void ) {
	...
	CG_ScaleFrom640( &x, &y, &w, &h, UI_CENTER );
	...
	aspectRatio = (float)cgs.glconfig.vidWidth / cgs.glconfig.vidHeight;

	if (aspectRatio > (float)4/3)
	{
		UIWidth = cgs.glconfig.vidHeight * (float)4/3;
		barWidth = ceil((float)(cgs.glconfig.vidWidth - UIWidth) / 2);
		CG_FillRect(0, 0, barWidth, cgs.glconfig.vidHeight, colorBlack, UI_DIRECT);
		CG_FillRect(barWidth+UIWidth - 1, 0, barWidth, cgs.glconfig.vidHeight, colorBlack, UI_DIRECT);
	}
	else
	{
		UIWidth = cgs.glconfig.vidWidth * (float)3/4;
		barWidth = ceil((float)(cgs.glconfig.vidHeight - UIWidth) / 2);
		CG_FillRect(0, 0, cgs.glconfig.vidHeight, barWidth, colorBlack, UI_DIRECT);
		CG_FillRect(0, barWidth+UIWidth - 1, cgs.glconfig.vidHeight, barWidth, colorBlack, UI_DIRECT);
	}	
	...
	if ( weap == WP_SNIPERRIFLE ) {
		...
		if ( cgs.media.reticleShaderSimpleQ ) {
			...
			trap_R_DrawStretchPic( x, h + y, w, h, 0, 1, 1, 0, cgs.media.reticleShaderSimpleQ );    // bl
			trap_R_DrawStretchPic( x + w, h + y, w, h, 1, 1, 0, 0, cgs.media.reticleShaderSimpleQ );  // br
		}
...
//----(SA)	end
		...
		if ( cgs.media.snooperShaderSimple ) {
			CG_DrawPic( 80, 0, 480, 480, cgs.media.snooperShaderSimple, UI_CENTER );
		}

		// hairs

		CG_FillRect( 310, 120, 20, 1, color, UI_CENTER );   //					-----
		CG_FillRect( 300, 160, 40, 1, color, UI_CENTER );   //				-------------
		CG_FillRect( 310, 200, 20, 1, color, UI_CENTER );   //					-----

		CG_FillRect( 140, 239, 360, 1, color, UI_CENTER );  // horiz ---------------------------

		CG_FillRect( 310, 280, 20, 1, color, UI_CENTER );   //					-----
		CG_FillRect( 300, 320, 40, 1, color, UI_CENTER );   //				-------------
		CG_FillRect( 310, 360, 20, 1, color, UI_CENTER );   //					-----



		CG_FillRect( 400, 220, 1, 40, color, UI_CENTER );   // l

		CG_FillRect( 319, 60, 1, 360, color, UI_CENTER );   // vert

		CG_FillRect( 240, 220, 1, 40, color, UI_CENTER );   // r
		...
		// hairs
		CG_FillRect( 84, 239, 150, 3, color, UI_CENTER );   // left
		CG_FillRect( 234, 240, 173, 1, color, UI_CENTER );  // horiz center
		CG_FillRect( 407, 239, 150, 3, color, UI_CENTER );  // right


		CG_FillRect( 319, 2,   3, 151, color, UI_CENTER );  // top center top
		CG_FillRect( 320, 153, 1, 114, color, UI_CENTER );  // top center bot

		CG_FillRect( 320, 241, 1, 87, color, UI_CENTER );   // bot center top
		CG_FillRect( 319, 327, 3, 151, color, UI_CENTER );  // bot center bot
	}
}
...
static void CG_DrawBinocReticle( void ) {
	...
	if ( cgs.media.binocShaderSimpleQ ) {
		...
		trap_R_DrawStretchPic( w+x, y, w, h, 1, 0, 0, 1, cgs.media.binocShaderSimpleQ );  // tr
		trap_R_DrawStretchPic( x, h+y, w, h, 0, 1, 1, 0, cgs.media.binocShaderSimpleQ );  // bl
		trap_R_DrawStretchPic( w+x, h+y, w, h, 1, 1, 0, 0, cgs.media.binocShaderSimpleQ );  // br
	}
	...
	CG_FillRect( 146, 239, 348, 1, color, UI_CENTER );

	CG_FillRect( 188, 234, 1, 13, color, UI_CENTER );   // ll
	CG_FillRect( 234, 226, 1, 29, color, UI_CENTER );   // l
	CG_FillRect( 274, 234, 1, 13, color, UI_CENTER );   // lr
	CG_FillRect( 320, 213, 1, 55, color, UI_CENTER );   // center
	CG_FillRect( 360, 234, 1, 13, color, UI_CENTER );   // rl
	CG_FillRect( 406, 226, 1, 29, color, UI_CENTER );   // r
	CG_FillRect( 452, 234, 1, 13, color, UI_CENTER );   // rr
}
...
//		CG_FillRect (319, 242, 2, 16, hcolor);	// vertical
		...
		// option 2
		CG_FillRect( 305, 240, 30, 2, hcolor, UI_CENTER );  // horizontal
		CG_FillRect( 314, 256, 12, 2, hcolor, UI_CENTER );  // horizontal2
		CG_FillRect( 319, 242, 2, 32, hcolor, UI_CENTER );  // vertical
	...
	case WP_KNIFE:
		...
		if ( !friendInSights ) {
			if ( !cg.snap->ps.leanf ) {     // no crosshair while leaning
				CG_FillRect( 319, 239, 2, 2, hcolor, UI_CENTER );      // dot
			}
			return;
		}
...
*/
	...
	CG_ScaleFrom640( &x, &y, &w, &h, UI_SCALEONLY );
...
//		CG_FillRect( -10, -10, 650, 490, col );
		CG_FillRect( 0, 0, 640, 480, col, UI_STRETCH ); // why do a bunch of these extend outside 640x480?
	}
}
...
//			Vector4Set( color, 0.5f, 0.3f, 0.5f, 1.0f - frac );
		...
		CG_FillRect( -10, -10, 650, 490, color, UI_STRETCH );
	}
}
...
static void CG_DrawFlashDamage( void ) {
	...
	if ( cg.v_dmg_time > cg.time ) {
		...
		CG_FillRect( -10, -10, 650, 490, col, UI_STRETCH );
	}
}
...
static void CG_DrawFlashFire( void ) {
	...
	if ( alpha > 0 ) {
		...
		col[3] = alpha;
		trap_R_SetColor( col );
		CG_DrawPic( -10, -10, 650, 490, cgs.media.viewFlashFire[( cg.time / 50 ) % 16], UI_CENTER );
		trap_R_SetColor( NULL );
...
static void CG_DrawFlashLightning( void ) {
	...
	if ( alpha > 0 ) {
		...
		CG_DrawPic( -10, -10, 650, 490, shader, UI_CENTER );
	}
}
...
static void CG_DrawObjectiveInfo( void ) {
	...
	backColor[3] = color[3];
	CG_FillRect( x1, y1, x2 - x1, y2 - y1, backColor, UI_CENTER );

	VectorSet( backColor, 0, 0, 0 );
	CG_DrawRect( x1, y1, x2 - x1, y2 - y1, 1, backColor, UI_CENTER );
...
static void CG_DrawGameScreenFade( void ) {
	...
	VectorClear( col );
	col[3] = cg.viewFade;
	CG_FillRect( 0, 0, 640, 480, col, UI_STRETCH );
}
...
static void CG_ScreenFade( void ) {
	...
	if ( msec <= 0 ) {
		...
		CG_FillRect( 0, 0, 640, 480, cg.fadeColor1, UI_STRETCH );
		...
		if ( color[ 3 ] ) {
			CG_FillRect( 0, 0, 640, 480, color, UI_STRETCH );
		}
	}
}
