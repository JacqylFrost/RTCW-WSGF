...
================
*/
void CG_ScaleFrom640( float *x, float *y, float *w, float *h, int mode ) {
#if 0
	...
	{
		if ((float) cgs.glconfig.vidWidth / cgs.glconfig.vidHeight > (float)4/3)
		{
			virtualXRes = ((float)cgs.glconfig.vidWidth / cgs.glconfig.vidHeight) * 480;
		...
		{
			virtualYRes = ((float) cgs.glconfig.vidHeight / cgs.glconfig.vidWidth) * 640;
	...
	else if (mode == UI_BOTTOM)
	{
		...
		{
			yoffset = 480 - *y;
			yoffset *= cgs.screenXScale;

			*x *= cgs.screenXScale;
			*y = cgs.glconfig.vidHeight - yoffset;
			*w *= cgs.screenXScale;
			*h *= cgs.screenXScale;
		}
	}
	...
	else if (mode == UI_SCALEONLY)
	{
		*w *= cgs.screenYScale;
		*h *= cgs.screenYScale;
	}
	else if (mode == UI_DIRECT)
		;
	else
	{
		*x = 0;
...
void CG_AdjustFrom640( float *x, float *y, float *w, float *h ) {
	CG_ScaleFrom640(x, y, w, h, UI_STRETCH);
}
...
=================
*/
void CG_FillRect( float x, float y, float width, float height, const float *color, int mode ) {
	trap_R_SetColor( color );

	CG_ScaleFrom640( &x, &y, &width, &height, mode );
...
void CG_FilledBar( float x, float y, float w, float h, const float *startColorIn, float *endColor, const float *bgColor, float frac, int flags ) {
	...
	if ( flags & BAR_VERT ) {
		...
		if ( flags & BAR_LERP_COLOR ) {
			CG_FillRect( x, y, w, h * frac, colorAtPos, UI_CENTER );
		} else {
//			CG_FillRectGradient ( x, y, w, h * frac, startColor, endColor, 0 );
			CG_FillRect( x, y, w, h * frac, startColor, UI_BOTTOM );
		}
		...
		if ( flags & BAR_LERP_COLOR ) {
			CG_FillRect( x, y, w * frac, h, colorAtPos, UI_CENTER );
		} else {
//			CG_FillRectGradient ( x, y, w * frac, h, startColor, endColor, 0 );
			CG_FillRect( x, y, w * frac, h, startColor, UI_CENTER );
		}
	}
...
================
*/
void CG_DrawSides( float x, float y, float w, float h, float size, int mode ) {
...
void CG_DrawTopBottom( float x, float y, float w, float h, float size, int mode ) {
	CG_ScaleFrom640( &x, &y, &w, &h, mode );
...
=================
*/
void CG_DrawRect( float x, float y, float width, float height, float size, const float *color, int mode ) {
	...
	CG_DrawTopBottom( x, y, width, height, size, mode );
	CG_DrawSides( x, y, width, height, size, mode );
...
=================
*/
void CG_DrawPic( float x, float y, float width, float height, qhandle_t hShader, int mode ) {
	CG_ScaleFrom640( &x, &y, &width, &height, mode );
...
void CG_DrawChar( int x, int y, int width, int height, int ch ) {
	...
	aw = width;
	ah = height;
	CG_ScaleFrom640( &ax, &ay, &aw, &ah, UI_CENTER );
...
void CG_DrawChar2( int x, int y, int width, int height, int ch ) {
	...
	aw = width;
	ah = height;
	CG_ScaleFrom640( &ax, &ay, &aw, &ah, UI_BOTTOM );
