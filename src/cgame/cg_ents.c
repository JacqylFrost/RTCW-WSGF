...
void CG_DrawHoldableSelect( void ) {
	...
	for ( i = 1 ; i <= HI_BOOK3 ; i++ ) {
		...
		if ( i == HI_WINE ) {
			...
			CG_DrawPic( x, y, 32, 32, cg_items[item - bg_itemlist].icons[ 2 - ( wine - 1 )], UI_CENTER ) ;
		} else {
			CG_DrawPic( x, y, 32, 32, cg_items[item - bg_itemlist].icons[0], UI_CENTER );
		}
		...
		if ( i == cg.holdableSelect ) {
			CG_DrawPic( x - 4, y - 4, 40, 40, cgs.media.selectShader, UI_CENTER );
		}
