...
static float CG_CalcFovVar(qboolean dead)
{
	static float lastfov = DEFAULT_FOV;
	float fov;
	float zoomFov;
	float f;
	...
	if ( cgs.gametype == GT_WOLF ) {
		if ( cg.snap->ps.pm_flags & PMF_FOLLOW && cg.snap->ps.weapon == WP_SNIPERRIFLE ) {
			fov = cg_zoomDefaultSniper.value;
		}
	}
	// dhm - end
...
static int CG_CalcFov( void ) {
	...
	{
		fov_y = CG_CalcFovVar(dead);
		yRatio = tan(fov_y/2 * (M_PI/180));
		aspectRatio = ((float)cg.refdef.width / cg.refdef.height);
		xRatio = yRatio * aspectRatio;
		fov_x = 2 * atan(xRatio) * (180/M_PI);
	}
	else
	{
		fov_x = CG_CalcFovVar(dead);
		xRatio = tan(fov_x/2 * (M_PI/180));
		aspectRatio = ((float)cg.refdef.width / cg.refdef.height);
		yRatio = xRatio / aspectRatio;
		fov_y = 2 * atan(yRatio) * (180/M_PI);
	}
...
static int CG_CalcViewValues( void ) {
	...
	if ( cg.cameraMode ) {
		...
		float fov_y;
		float fov_x;
		float fovVar = DEFAULT_FOV;
		float yRatio;
		float xRatio;
		float aspectRatio;

		if ( trap_getCameraInfo( CAM_PRIMARY, cg.time, &origin, &angles, &fovVar ) ) {
