...
================
*/
void UI_ScaleFrom640( float *x, float *y, float *w, float *h, int mode ) {
	...
	*h *= uiInfo.uiDC.scale;
	...
	{
		if ((float) uiInfo.uiDC.glconfig.vidWidth / uiInfo.uiDC.glconfig.vidHeight > (float)4/3)
		{
			virtualXRes = ((float)uiInfo.uiDC.glconfig.vidWidth / uiInfo.uiDC.glconfig.vidHeight) * 480;
			xoffset = (virtualXRes-640)/2;
				
			*x += xoffset;
			*x *= uiInfo.uiDC.yscale;
			*y *= uiInfo.uiDC.yscale;
			*w *= uiInfo.uiDC.yscale;
			*h *= uiInfo.uiDC.yscale;
		}
		else
		{
			virtualYRes = ((float) uiInfo.uiDC.glconfig.vidHeight / uiInfo.uiDC.glconfig.vidWidth) * 640;
			yoffset = (virtualYRes-480)/2;
				
			*x *= uiInfo.uiDC.xscale;
			*y += yoffset;
			*y *= uiInfo.uiDC.xscale;
			*w *= uiInfo.uiDC.xscale;
	...
	else if (mode == UI_BOTTOM)
	{
		...
		{
			yoffset = 480 - *y;
			yoffset *= uiInfo.uiDC.xscale;

			*x *= uiInfo.uiDC.xscale;
			*y = uiInfo.uiDC.glconfig.vidHeight - yoffset;
			*w *= uiInfo.uiDC.xscale;
			*h *= uiInfo.uiDC.xscale;
		}
	}
...
void UI_AdjustFrom640( float *x, float *y, float *w, float *h ) {
	UI_ScaleFrom640(x, y, w, h, UI_STRETCH);
}
...
void UI_DrawHandlePic( float x, float y, float w, float h, qhandle_t hShader, int mode ) {
	float s0;
	float s1;
	float t0;
	float t1;
...
=================
*/
void UI_FillRect( float x, float y, float width, float height, const float *color, int mode ) {
	trap_R_SetColor( color );

	UI_ScaleFrom640( &x, &y, &width, &height, mode );
...
=================
*/
void UI_DrawRect( float x, float y, float width, float height, const float *color, int mode ) {
	trap_R_SetColor( color );
...
void UI_DrawTextBox( int x, int y, int width, int lines ) {
	UI_FillRect( x + BIGCHAR_WIDTH / 2, y + BIGCHAR_HEIGHT / 2, ( width + 1 ) * BIGCHAR_WIDTH, ( lines + 1 ) * BIGCHAR_HEIGHT, colorBlack, UI_CENTER );
	UI_DrawRect( x + BIGCHAR_WIDTH / 2, y + BIGCHAR_HEIGHT / 2, ( width + 1 ) * BIGCHAR_WIDTH, ( lines + 1 ) * BIGCHAR_HEIGHT, colorWhite, UI_CENTER );
}
