...
void GradientBar_Paint( rectDef_t *rect, vec4_t color ) {
	...
	DC->setColor( color );
	DC->drawHandlePic( rect->x, rect->y, rect->w, rect->h, DC->Assets.gradientBar, UI_CENTER );
	DC->setColor( NULL );
}
...
void Window_Paint( Window *w, float fadeAmount, float fadeClamp, float fadeCycle ) {
	...
	if ( debugMode ) {
		...
		DC->drawRect( w->rect.x, w->rect.y, w->rect.w, w->rect.h, 1, color, UI_CENTER );
	}
	...
	if ( w->style == WINDOW_STYLE_FILLED ) {
		...
		if ( w->background ) {
			...
			DC->drawHandlePic( fillRect.x, fillRect.y, fillRect.w, fillRect.h, w->background, UI_CENTER );
			DC->setColor( NULL );
		} else {
			DC->fillRect( fillRect.x, fillRect.y, fillRect.w, fillRect.h, w->backColor, UI_CENTER );
		}
		...
		if ( DC->getTeamColor ) {
			...
			DC->fillRect( fillRect.x, fillRect.y, fillRect.w, fillRect.h, color, UI_CENTER );
		}
	...
	if ( w->border == WINDOW_BORDER_FULL ) {
		...
		if ( w->style == WINDOW_STYLE_TEAMCOLOR ) {
			...
			color[3] = 1;
			DC->drawRect( w->rect.x, w->rect.y, w->rect.w, w->rect.h, w->borderSize, color, UI_CENTER );
		} else 
		{
			if (w->name[0] == 'l' ||	// "load_menu"
				w->name[0] == 'w' ||	// "window"
				w->name[0] == 'a' ||	// "autosave_off"
				w->name[0] == 'n' ||	// "notebook"
				w->name[0] == 'p' ||	// "play_menu"
				w->name[0] == 'm' ||	// "mappreview"
				w->name[0] == 'c' ||	// "ctr_look"
				w->name[0] == 'g' ||	// "graphicsapply"
				w->name[0] == 'y' ||	// "yes"
				w->name[0] == 'o' ||	// "options_menu"
				w->name[0] == 'd' ||	// "default_menu"
				w->name[0] == 'r' ||	// "rec_restart_popmenu"
				w->name[0] == 'i' ||	// "ingame_leave"
				w->name[0] == 'q' ||	// "quit_popmenu"
				w->name[0] == 's')		// "setup_menu"
			{
				DC->drawRect( w->rect.x, w->rect.y, w->rect.w, w->rect.h, w->borderSize, w->borderColor, UI_CENTER );
			}
			else
				DC->drawRect( w->rect.x, w->rect.y, w->rect.w, w->rect.h, w->borderSize, w->borderColor, UI_BOTTOM );
		}
	} else if ( w->border == WINDOW_BORDER_HORZ ) {
		// top/bottom
		DC->setColor( w->borderColor );
		DC->drawTopBottom( w->rect.x, w->rect.y, w->rect.w, w->rect.h, w->borderSize, UI_CENTER );
		DC->setColor( NULL );
	} else if ( w->border == WINDOW_BORDER_VERT ) {
		// left right
		DC->setColor( w->borderColor );
		DC->drawSides( w->rect.x, w->rect.y, w->rect.w, w->rect.h, w->borderSize, UI_CENTER );
		DC->setColor( NULL );
	...
	while ( p ) {
		...
		if ( ( newLine && textWidth > item->window.rect.w ) || *p == '\n' || *p == '\0' ) {
			if ( len ) {
				...
				//
				buff[newLine] = '\0';
				DC->drawText( item->textRect.x, item->textRect.y, item->font, item->textscale, color, buff, 0, 0, item->textStyle, UI_CENTER );
			}
			if ( *p == '\0' ) {
				break;
			}
...
void Item_Text_Wrapped_Paint( itemDef_t *item ) {
	...
	while ( p && *p ) {
		...
		buff[p - start] = '\0';
		DC->drawText( x, y, item->font, item->textscale, color, buff, 0, 0, item->textStyle, UI_CENTER );
		y += height + 5;
		start += p - start + 1;
		p = strchr( p + 1, '\r' );
	}
	DC->drawText( x, y, item->font, item->textscale, color, start, 0, 0, item->textStyle, UI_CENTER );
}
...
//	}
	...
	DC->drawText( item->textRect.x, item->textRect.y, item->font, item->textscale, color, textPtr, 0, 0, item->textStyle, UI_CENTER );
}
...
void Item_TextField_Paint( itemDef_t *item ) {
	...
	if ( item->window.flags & WINDOW_HASFOCUS && g_editingField ) {
		...
		DC->drawText( item->textRect.x + item->textRect.w + offset, item->textRect.y, item->font, item->textscale, newColor, buff + editPtr->paintOffset, 0, editPtr->maxPaintChars, item->textStyle, UI_CENTER );
	}
...
void Item_YesNo_Paint( itemDef_t *item ) {
	...
	if ( item->text ) {
		Item_Text_Paint( item );
		DC->drawText( item->textRect.x + item->textRect.w + 8, item->textRect.y, item->font, item->textscale, newColor, ( value != 0 ) ? yes_str : no_str, 0, 0, item->textStyle, UI_CENTER );
	} else {
		DC->drawText( item->textRect.x, item->textRect.y, item->font, item->textscale, newColor, ( value != 0 ) ? yes_str : no_str, 0, 0, item->textStyle, UI_CENTER );
	}
}
...
void Item_Multi_Paint( itemDef_t *item ) {
	...
	if ( item->text ) {
		Item_Text_Paint( item );
		DC->drawText( item->textRect.x + item->textRect.w + 8, item->textRect.y, item->font, item->textscale, newColor, text, 0, 0, item->textStyle, UI_CENTER );
	} else {
		DC->drawText( item->textRect.x, item->textRect.y, item->font, item->textscale, newColor, text, 0, 0, item->textStyle, UI_CENTER );
	}
}
...
void Item_Slider_Paint( itemDef_t *item ) {
	...
	DC->drawHandlePic( x, y, SLIDER_WIDTH, SLIDER_HEIGHT, DC->Assets.sliderBar, UI_CENTER );

	x = Item_Slider_ThumbPosition( item );
	DC->drawHandlePic( x - ( SLIDER_THUMB_WIDTH / 2 ), y - 2, SLIDER_THUMB_WIDTH, SLIDER_THUMB_HEIGHT, DC->Assets.sliderThumb, UI_CENTER );
}
...
void Item_Bind_Paint( itemDef_t *item ) {
	...
	if ( item->text ) {
		...
		DC->drawText( item->textRect.x + item->textRect.w + 8, item->textRect.y, item->font, item->textscale, newColor, g_nameBind1, 0, maxChars, item->textStyle, UI_CENTER );
	} else {
		DC->drawText( item->textRect.x, item->textRect.y, item->font, item->textscale, newColor, ( value != 0 ) ? "FIXME" : "FIXME", 0, maxChars, item->textStyle, UI_CENTER );
	}
}
...
void ScaleFrom640( float *x, float *y, float *w, float *h, int mode ) {
	...
	else if (mode == UI_CENTER)
	{
		if ((float) DC->glconfig.vidWidth / DC->glconfig.vidHeight > (float)4/3)
		{
			virtualXRes = ((float) DC->glconfig.vidWidth / DC->glconfig.vidHeight) * 480;
			xoffset = (virtualXRes-640)/2;
		...
		{
			virtualYRes = ((float) DC->glconfig.vidHeight / DC->glconfig.vidWidth) * 640;
			yoffset = (virtualYRes-480)/2;
...
{
	ScaleFrom640(x, y, w, h, UI_STRETCH);
}
...
void Item_Model_Paint( itemDef_t *item ) {
	...
	ScaleFrom640( &x, &y, &w, &h, UI_CENTER );
...
void Item_Image_Paint( itemDef_t *item ) {
	...
	DC->drawHandlePic( item->window.rect.x + 1, item->window.rect.y + 1, item->window.rect.w - 2, item->window.rect.h - 2, item->asset, UI_CENTER );
}
...
void Item_ListBox_Paint( itemDef_t *item ) {
	...
	if ( item->window.flags & WINDOW_HORIZONTAL ) {
		...
		DC->drawHandlePic( x, y, SCROLLBAR_SIZE, SCROLLBAR_SIZE, DC->Assets.scrollBarArrowLeft, UI_CENTER );
		x += SCROLLBAR_SIZE - 1;
		size = item->window.rect.w - ( SCROLLBAR_SIZE * 2 );
		DC->drawHandlePic( x, y, size + 1, SCROLLBAR_SIZE, DC->Assets.scrollBar, UI_CENTER );
		x += size - 1;
		DC->drawHandlePic( x, y, SCROLLBAR_SIZE, SCROLLBAR_SIZE, DC->Assets.scrollBarArrowRight, UI_CENTER );
		// thumb
		thumb = Item_ListBox_ThumbDrawPosition( item ); //Item_ListBox_ThumbPosition(item);
		if ( thumb > x - SCROLLBAR_SIZE - 1 ) {
			thumb = x - SCROLLBAR_SIZE - 1;
		}
		DC->drawHandlePic( thumb, y, SCROLLBAR_SIZE, SCROLLBAR_SIZE, DC->Assets.scrollBarThumb, UI_CENTER );
		//
		...
		if ( listPtr->elementStyle == LISTBOX_IMAGE ) {
			...
			for ( i = listPtr->startPos; i < count; i++ ) {
				...
				if ( image ) {
					DC->drawHandlePic( x + 1, y + 1, listPtr->elementWidth - 2, listPtr->elementHeight - 2, image, UI_CENTER );
				}

				if ( i == item->cursorPos ) {
					DC->drawRect( x, y, listPtr->elementWidth - 1, listPtr->elementHeight - 1, item->window.borderSize, item->window.borderColor, UI_CENTER );
				}
		...
		y = item->window.rect.y + 1;
		DC->drawHandlePic( x, y, SCROLLBAR_SIZE, SCROLLBAR_SIZE, DC->Assets.scrollBarArrowUp, UI_CENTER );
		y += SCROLLBAR_SIZE - 1;

		listPtr->endPos = listPtr->startPos;
		size = item->window.rect.h - ( SCROLLBAR_SIZE * 2 );
		DC->drawHandlePic( x, y, SCROLLBAR_SIZE, size + 1, DC->Assets.scrollBar, UI_CENTER );
		y += size - 1;
		DC->drawHandlePic( x, y, SCROLLBAR_SIZE, SCROLLBAR_SIZE, DC->Assets.scrollBarArrowDown, UI_CENTER );
		// thumb
		thumb = Item_ListBox_ThumbDrawPosition( item ); //Item_ListBox_ThumbPosition(item);
		if ( thumb > y - SCROLLBAR_SIZE - 1 ) {
			thumb = y - SCROLLBAR_SIZE - 1;
		}
		DC->drawHandlePic( x, thumb, SCROLLBAR_SIZE, SCROLLBAR_SIZE, DC->Assets.scrollBarThumb, UI_CENTER );
			...
			for ( i = listPtr->startPos; i < count; i++ ) {
				...
				if ( listPtr->numColumns > 0 ) {
					...
					for ( j = 0; j < listPtr->numColumns; j++ ) {
						...
						if ( optionalImage >= 0 ) {
							DC->drawHandlePic( x + 4 + listPtr->columnInfo[j].pos, y - 1 + listPtr->elementHeight / 2, listPtr->columnInfo[j].width, listPtr->columnInfo[j].width, optionalImage, UI_CENTER );
						} else if ( text ) {
							DC->drawText( x + 4 + listPtr->columnInfo[j].pos, y + listPtr->elementHeight, item->font, item->textscale, item->window.foreColor, text, 0, listPtr->columnInfo[j].maxChars, item->textStyle, UI_CENTER );
						}
					}
				} else {
					...
					if ( optionalImage >= 0 ) {
						...
						DC->drawText( x + 4, y + listPtr->elementHeight, item->font, item->textscale, item->window.foreColor, text, 0, 0, item->textStyle, UI_CENTER );
					}
				}

				if ( i == item->cursorPos ) {
					DC->fillRect( x + 2, y + 2, item->window.rect.w - SCROLLBAR_SIZE - 4, listPtr->elementHeight, item->window.outlineColor, UI_CENTER );
				}
...
void Item_Paint( itemDef_t *item ) {
	...
	if ( debugMode ) {
		...
		color[1] = color[3] = 1;
		color[0] = color[2] = 0;
		DC->drawRect( r->x, r->y, r->w, r->h, 1, color, UI_CENTER );
	}
...
void Menu_Paint( menuDef_t *menu, qboolean forcePaint ) {
	...
	if ( menu->fullScreen ) {
		...
		DC->drawHandlePic( 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, menu->window.background, UI_CENTER );
	...
	if ( debugMode ) {
		...
		color[1] = 0;
		DC->drawRect( menu->window.rect.x, menu->window.rect.y, menu->window.rect.w, menu->window.rect.h, 1, color, UI_CENTER );
	}
}
...
void Menu_PaintAll(qboolean pillarbox) {
	int i;
	int barWidth;
	int UIWidth;
	float aspectRatio;
	...
	if (pillarbox)
	{
		...
		if (aspectRatio > (float)4/3)
		{
			UIWidth = DC->glconfig.vidHeight * (float)4/3;
			barWidth = (DC->glconfig.vidWidth - UIWidth) / 2;
			DC->fillRect(0, 0, barWidth, DC->glconfig.vidHeight, colorBlack, UI_DIRECT);
			DC->fillRect(barWidth+UIWidth, 0, barWidth, DC->glconfig.vidHeight, colorBlack, UI_DIRECT);
		}
		else
		{
			UIWidth = DC->glconfig.vidWidth * (float)3/4;
			barWidth = (DC->glconfig.vidHeight - UIWidth) / 2;
			DC->fillRect(0, 0, DC->glconfig.vidWidth, barWidth, colorBlack, UI_DIRECT);
			DC->fillRect(0, barWidth+UIWidth, DC->glconfig.vidWidth, barWidth, colorBlack, UI_DIRECT);
		}
	}

	if ( debugMode ) {
		vec4_t v = {1, 1, 1, 1};
		DC->drawText( 5, 25, 0, .5, v, va( "fps: %f", DC->FPS ), 0, 0, 0, UI_CENTER );
	}
}
